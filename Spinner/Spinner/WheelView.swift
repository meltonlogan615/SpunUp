...
extension WheelView {
  func performSelection() {
    ...
    let fastClacks = clackyClack(speed: .fast, duration: fast.duration)
    wheelView.layer.add(fast, forKey: "fastAnimation")
    fastClacks.invalidate()
    let slowClacks = clackyClack(speed: .slow, duration: slow.duration)
    wheelView.layer.add(slow, forKey: "SlowAnimation")
    slowClacks.invalidate()
